...
function cycleFormed(i,j,matrix){
    const der = matrix[i-1][j-1]!==Infinity
    return der
}

function repair(edgesToRepair,matrix) {
...
function solve(numNodes, edges, edgedToRepair) {
    ...
    return repair(edgedToRepair,matrix)
}
let n = 5
...
console.log(solve(n,edgez,repairMe))
